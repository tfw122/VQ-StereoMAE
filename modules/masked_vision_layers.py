...
class MAEDecoder(nn.Module):
    def __init__(self, config, patch_embed, norm_layer):
        ...
        self.decoder_pred = nn.Linear(decoder_embed_dim, patch_size[0]*patch_size[1] * in_channels, bias=True) # decoder to patch
...
class MSGMAEDecoder(nn.Module):
    def __init__(self, config, patch_embed, norm_layer):
        ...
        self.decoder_norm = norm_layer(decoder_embed_dim)
        #self.decoder_pred = nn.Linear(decoder_embed_dim, patch_size**2 * in_channels, bias=True) # decoder to patch
        self.decoder_pred = nn.Linear(decoder_embed_dim, patch_size[0]*patch_size[0] * in_channels, bias=True) # decoder to patch
